...
Engine version: **0.7.4** (see `CHANGELOG.md`). Install it with `uv sync` in this repository; that puts `tos-config`, `tos-init`, `tos-lint` and `tos-deny` on `uv run`.
...
11. Connectors are read-only and used only by `/tos-pull`, the cross-check pass and the drift check. Never post, comment, react, transition or edit in any connected system. `.claude/settings.json` denies the write-capable tools of the connectors this engine names, so the rule is enforced and not merely instructed. A permission rule matches a tool by its exact name, so run `uv run tos-deny` after adding or renaming a connector: it crosses the `mcp_server` names in your config with `schema/connector-writes.yaml` and reports what is not yet denied, and `--write` adds it to `.claude/settings.local.json`.
