...
- Objective is a phase-1 type, with `level` (company or team), `team` on the team-level ones so several teams' OKRs can sit side by side, and `quarter`, with quarter-prefixed slugs. Projects link the objective they advance from *Expected impact*; a team objective links its company objective. Both are ordinary body links — no new frontmatter relation.
