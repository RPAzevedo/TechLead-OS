...
3. **Claude Code.** Start it in this directory so `CLAUDE.md` loads, and grant it the data root: `claude --add-dir <data.root>`. To stop repeating the flag, put the absolute path in `permissions.additionalDirectories` in `.claude/settings.local.json` — untracked and per-machine, so it does not exist until you write it. Connectors are MCP servers configured in Claude Code. The config's `connectors.<name>.mcp_server` must match their names as `claude mcp list` prints them — `provider` is the *kind* of connector and selects a write-tool vocabulary, not a server. Run `uv run tos-deny` after editing either.
