...
The design, with its decisions, is in `docs/design.html` (also `design.pdf`, `design.md`); eight usage scenarios are in `docs/scenarios.html`. This README is the quickstart.
...
├── config.example.yaml      copy to ~/.config/commonplace/config.yaml
...
├── scripts/pos_common.py    config + frontmatter helpers (PyYAML optional)
...
1. **Config.** `mkdir -p ~/.config/commonplace && cp config.example.yaml ~/.config/commonplace/config.yaml`, then edit `data.root`, `data.actor`, and the connector scopes you have. It holds no secrets. `python3 scripts/pos_common.py --show` prints what the engine resolved.
