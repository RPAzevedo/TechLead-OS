---
description: Health-check the bundle — deterministic script, then the agent pass
argument-hint: [--fix]
---
Follow CLAUDE.md §0, then §4.5.

1. Run `uv run lint` and read the report.
2. Agent pass, on the pages the report mentions plus any page changed in the last 7 days (use `git log` in `data.root`): contradictions between pages; claims without a footnote; missing cross-references between pages that mention each other's subjects; gaps worth a `Question` page; the people policy (CLAUDE.md §5) on any page that mentions a person; and **source drift**: for each Source page whose `sources[].resource` is a connector pointer — a URL, a Jira issue key or JQL, a Slack channel window — that passes the checks of `/tos-pull` step 2 (a local path inside `md`'s repos; `trello` from phase 4 and within its boards; never a Slack DM), fetch the source's current modified time or version through the connector (read-only; for a JQL query or a channel window, the newest `updated` time or message) and list those newer than the recorded `last_modified` as "changed since read".
3. Write the combined findings as the *Lint* section of the current week's Review page (`wiki/reviews/<ISO-week>.md`; create it from `schema/templates/review.md` if it does not exist). Do not fix anything judgement-shaped.
4. Only if `--fix` was given: run `uv run lint --fix` — it repairs the mechanical findings (missing index entries, dead index lines, moved and leading-slash links) and reports what it fixed; repair by hand only what needs judgement and the script left alone. Then `uv run log Lint "fixed N mechanical findings; M open."` and commit. Lint never adds `verified` entries.

Finish with counts per check and the three findings you would act on first.
