...
1. Run `uv run tos-lint` and read the report.
...
4. Only if `--fix` was given: run `uv run tos-lint --fix` — it repairs the mechanical findings (missing index entries, dead index lines, moved and leading-slash links) and reports what it fixed; repair by hand only what needs judgement and the script left alone. Then `uv run tos-log Lint "fixed N mechanical findings; M open."` and commit. Lint never adds `verified` entries.
