...
7. Log under today's date in `wiki/log.md`: `* **Pull**: <connector> <short pointer> → [Source](sources/<file>), not pinned` (or `pinned`). Then the `Ingest` line(s) from step 4.
