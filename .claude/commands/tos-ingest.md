...
5. New pages are created with `uv run new <Type> <slug> --title "…" --description "…" --by claude-code/<your model id>` — it copies the template, computes `generated`, `status: draft` and `stale_after` from the registry, and indexes the page; you then fill the body. On a page you update by hand, set `generated: { by: claude-code/<your model id>, at: <now, ISO-8601 with offset> }` and re-run `uv run index <path>` so the index entry matches. **Never write `verified`.**
6. Append the operation's log line with `uv run log Ingest "[Title](sources/<file>) — touched N pages (M new)."`
